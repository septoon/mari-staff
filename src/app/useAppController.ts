...
export function useAppController(): AppController {
  ...
  const removeScheduleBreakForStaff = async ({
    staffId,
    date,
    start,
    end,
    ...
    try {
      ...
      const fallbackIntervals = workingHoursByStaff[staffId]?.[isoDate] ?? [];
      const intervals = dailyIntervals.length > 0 ? dailyIntervals : fallbackIntervals;
      const nextIntervals = restoreBreakInScheduleIntervals(intervals, start, end);

      if (!nextIntervals) {
        setToast('Перерыв с таким диапазоном не найден');
        return false;
      }
      if (nextIntervals.some((interval) => !isScheduleIntervalValid(interval))) {
        setToast('Не удалось убрать перерыв из этого графика');
        return false;
      }

      await putStaffDailySchedule(staffId, date, nextIntervals);
      await loadWorkingHours(staff);
      setToast(`Перерыв убран для ${targetStaff.name}`);
      return true;
    } catch (error) {
      setToast(toErrorMessage(error));
      return false;
